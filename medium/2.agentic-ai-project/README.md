...
### 5. Config 및 Observability 기능
#### 5.1 프로젝트 Config
config.py 파일은 전체 GenAI 파이프라인의 중앙 제어 계층 역할을 하여, 모든 구성 요소가 일관된 설정과 공유된 구성으로 작동하도록 보장

- API 키, 모델 매개변수 및 임베딩 구성을 관리
- 데이터베이스, PDF 및 FAISS 인덱스 저장을 위한 데이터 경로를 정의
- 청크 크기, 오버랩, 상위 k개 검색과 같은 RAG 동작을 구성
- SQL, RAG 및 웹 검색 도구 전반에 걸친 라우팅 옵션을 지정

#### 5.2 Observability (MLflow 추적 래퍼)
...
### 6. 도구 정의
#### 6.1 RAG 도구
- PDF 문서를 읽어들이고, 이를 중첩되는 청크로 분할한 뒤, 로컬 문장 변환 모델을 사용하여 임베딩으로 변환.
- 효율적인 의미적 유사도 검색을 위해 임베딩을 FAISS 벡터 인덱스에 저장.
- 쿼리 시점에 사용자의 질문을 기반으로 가장 관련성 높은 청크를 검색하여 구체적인 맥락을 제공.
- 검색된 텍스트와 원본 메타데이터를 반환하며, 최종 답변 생성은 후속 LLM 구성 요소에 맡김.
...
- 상세한 스키마와 조인 관계를 기반으로 LLM을 사용하여 자연어 질문을 SQLite SQL 쿼리로 변환.
- 생성된 SQL을 로컬 데이터베이스에 실행하고 구조화된 결과를 반환.
- 쿼리 출력을 하류 시스템에서 활용하기 쉬운 가독성 높은 마크다운 테이블 형식으로 변환.
- 오류를 원활하게 처리하며, 가시성 확보 및 디버깅을 위해 생성된 SQL과 실행 결과를 모두 제공.
  
/tools/sql_tool.py 파일 참고할 것

#### 6.3 Websearch Tool
- Serper API를 사용하여 웹에서 실시간 정보를 가져와 구조화된 결과(제목, URL, 요약문)를 반환.
- 내부 데이터나 RAG 컨텍스트를 사용하여 답변할 수 없는 쿼리에 대한 대체 수단으로 기능.
- 후속 합성 및 추론을 위해 LLM에 최적화된 깔끔한 검색 결과를 반환.
...
### 9. Finally main.py
main.py 파일은 애플리케이션의 진입점 역할을 하며, 세션을 초기화하고 필수 구성을 검증하는 동시에 GenAI 에이전트와 상호작용할 수 있는 대화형 CLI 및 데모 모드를 제공.

- SQL, RAG 및 웹 검색 경로를 테스트하기 위해 대화형 및 스크립트 기반 데모 모드를 모두 지원.
- LangGraph 파이프라인을 위한 세션 관리, 환경 검증 및 그래프 시각화를 처리.
...
그러면 3개의 쿼리가 자동으로 실행되며, 이 모든 내용은 MLFlow에 기록됨. 각 요청은 MLFlow UI에서 완벽하게 추적 가능하며, 모든 의사 결정, 도구 호출 및 LLM 상호 작용이 표시됨.
