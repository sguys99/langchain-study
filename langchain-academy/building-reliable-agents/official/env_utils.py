...
def _parse_required_keys(example_file_path: str) -> dict:
    """Parse example.env to identify required keys and their example values.
...
def check_env_file_exists(env_file_path: str = ".env", example_file_path: str = "example.env"):
    """Check if .env file exists. If not, check if required variables are in system env.

    Args:
        env_file_path: Path to the .env file
        example_file_path: Path to the example.env file
    Returns:
    ...
    if not os.path.exists(example_file_path):
        print("⚠️  No .env file found and no example.env to check against")
        print(f"   Run: cp example.env .env")
        print()
        return False
    ...
    for key, example_val in required_keys.items():
        ...
        if sys_val is not None:
            ...
            if "your_" in sys_val or "_here" in sys_val:
                missing.append(key)
            else:
                found[key] = sys_val
        else:
...
def doublecheck_env(file_path: str):
    ...
    for key in parsed.keys():
        ...
        if current is not None:
            ...
            # Only flag values that look like actual placeholders (e.g. "your_xxx_here")
            if key in required_keys:
                if current == example_val and example_val and ("your_" in example_val or "_here" in example_val):
...
if __name__ == "__main__":
    ...
    env_exists = check_env_file_exists(".env", "example.env")
