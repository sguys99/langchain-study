...
# v4 특징: 문서를 chunk로 쪼개지 않고 "문서 전체"를 하나의 단위로 임베딩.
...
def _embeddings_are_stale(kb_path: Path, cache_path: Path) -> bool:
    """
    임베딩 캐시가 만료(stale)되었는지 확인.
    캐시 생성 시점 이후에 수정된 문서가 하나라도 있으면 True를 반환하여
    임베딩을 다시 생성하도록 유도한다.
    """
    ...
    for file_path in kb_path.glob("*.md"):
        ...
        if file_path.stat().st_mtime > cache_mtime:
            print(f"  Stale: {file_path.name}이(가) 임베딩 생성 이후 수정되었습니다")
            return True
...
async def _generate_and_cache_embeddings(kb_path: Path, cache_path: Path) -> None:
    """
    모든 문서에 대해 임베딩을 생성하고 캐시(JSON)에 저장.
    v4는 chunk로 나누지 않고 "문서 전체"를 하나의 임베딩으로 만든다.
    """
    ...
    # 각 문서 전체에 대해 Voyage AI로 임베딩 생성
    # input_type="document": 인덱싱(저장)할 때 사용 (검색 시 "query"와 구분)
...
async def load_knowledge_base(kb_dir: str = "./knowledge_base") -> None:
    ...
    원본 문서가 수정되었다면 임베딩을 자동으로 재생성한다.
    임베딩 생성에는 비용/시간이 들기 때문에 캐싱과 stale 검사가 중요.
    """
    global knowledge_base_docs, knowledge_base_embeddings
...
async def search_knowledge_base(query: str, top_k: int = 2) -> str:
    ...
    # 4) 문서 "전체"를 LLM이 읽기 좋은 텍스트 형태로 포맷
    results = []
...
async def main():
    ...
    # 시작 시 knowledge base 로드 (캐시 사용 또는 문서 변경 시 재생성)
