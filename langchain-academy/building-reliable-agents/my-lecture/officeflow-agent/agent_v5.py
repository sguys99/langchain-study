...
# ===== Knowledge Base 저장소 (앱 시작 시 로드) =====
# v5의 특징: 문서를 chunk로 나누지 않고 "문서 전체"를 하나의 단위로 임베딩/검색함
# - knowledge_base_docs: (파일명, 본문) 튜플 리스트
# - knowledge_base_embeddings: 각 문서에 대응되는 임베딩 벡터(부동소수점 리스트)
knowledge_base_docs: List[Tuple[str, str]] = []  # (filename, content) 튜플 리스트
knowledge_base_embeddings: List[List[float]] = []  # 각 문서에 대응되는 임베딩
...
간결성 우선:
응답은 짧고 핵심을 찔러야 합니다. 불필요한 군더더기, 반복, 지나치게 장황한 설명은 피하세요. 곧바로 답을 제시하세요. 한 문장으로 말할 수 있다면 세 문장을 쓰지 마세요. 고객은 길고 장황한 응답보다 빠르고 직접적인 답변을 선호합니다.
...
중요 - 재고 정보 안내 정책:
제품 가용성을 안내할 때는 절대로 구체적인 재고 수량이나 숫자를 고객에게 공개하지 마세요. 대신 다음 가이드라인을 따르세요:
- 재고 수량 > 20: "재고가 충분합니다" 또는 "구매 가능합니다"로 안내
- 재고 수량 10-20: "재고가 있지만 빠르게 소진되고 있습니다" 또는 "구매 가능하나 재고가 제한적입니다"로 안내하여 구매 동기를 부여
- 재고 수량 5-9: "재고가 얼마 남지 않았습니다" 또는 "재고가 제한적입니다"로 안내하여 신속한 결정을 유도
- 재고 수량 1-4: "재고가 매우 적습니다" 또는 "거의 매진되었습니다"로 안내
- 재고 수량 0: "현재 재고가 없습니다" 또는 "현재는 구매가 불가능합니다"로 안내

이 정책은 회사의 경쟁 우위와 재고 관리 전략을 보호하면서도 고객이 정보에 기반한 구매 결정을 내릴 수 있도록 돕기 위한 것입니다.
...
고객: "복사 용지 있나요?"
당신: "네, 있습니다! 다양한 종류의 복사 용지를 취급하고 있어요. 표준 8.5x11인치 레터 사이즈를 찾으시나요, 아니면 특정 무게나 마감재가 필요하신가요?"

고객: "주문을 반품하고 싶어요"
당신: "반품은 반품 부서에서 처리합니다 - returns@officeflow.com 또는 1-800-OFFICE-1 내선 3번으로 연락하시면 됩니다. 영업일 기준 4시간 내에 답변을 드려요. 다른 도움이 필요하신가요?"
...
def _embeddings_are_stale(kb_path: Path, cache_path: Path) -> bool:
    """
    문서 중 하나라도 임베딩 캐시 생성 이후에 수정되었는지 확인.
    캐시가 없거나 어떤 문서가 캐시보다 최신이면 True(재생성 필요)를 반환.
    """
...
async def _generate_and_cache_embeddings(kb_path: Path, cache_path: Path) -> None:
    """
    모든 문서에 대한 임베딩을 생성하고 캐시에 저장.
    v5의 특징: chunk로 분할하지 않고 "문서 전체"를 한 단위로 임베딩함.
...
async def load_knowledge_base(kb_dir: str = "./knowledge_base") -> None:
    """
    knowledge base 문서와 임베딩을 로드 (chunk 분할 없이 "문서 전체" 단위).

    소스 문서가 수정되었으면 임베딩을 자동으로 재생성함.
    ...
    # 임베딩 재생성이 필요한지 확인 (문서가 캐시보다 최신이면 재생성)
...
async def search_knowledge_base(query: str, top_k: int = 2) -> str:
    """
    의미 기반 유사도(코사인 유사도)로 knowledge base를 검색.
    chunk가 아닌 "문서 전체"를 반환함.
